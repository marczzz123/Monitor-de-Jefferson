    ...
    <View style={styles.dotWrap}>
      <Animated.View
        style={[
          styles.dotRing,
          {
            backgroundColor: (active ? colors.success : colors.mutedForeground) + "30",
...
export default function DashboardScreen() {
  ...
  const analyzeCurrentApp = useCallback(async () => {
    ...
    try {
      ...
      const result = await analyzeApp({
        ...
        isSchoolHours: hour >= schedule.schoolStart && hour < schedule.schoolEnd,
        isNightTime: hour >= schedule.bedtime,
        restrictedApps,
      });
    ...
    setAnalyzing(false);
  }, [currentApp, analyzing, todayUsage, schedule, restrictedApps, addAction]);
  ...
  const decisionColor =
    lastDecision?.decision === "close"
      ? colors.destructive
      : lastDecision?.decision === "warn"
      ? colors.warning
      : colors.success;
      ...
      >
        <View style={[styles.currentAppCard, { backgroundColor: colors.primary }]}>
          ...
          {isMonitoring && currentApp ? (
            <>
              ...
              {lastDecision && (
                <Text style={styles.currentReason} numberOfLines={2}>
                  {lastDecision.reason}
                </Text>
              )}
            </>
            ...
            <Text style={styles.noAppText}>
              {isMonitoring ? "Esperando actividad..." : "Inicia el monitoreo"}
            </Text>
          )}
        </View>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={[styles.statCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <Text style={[styles.statLabel, { color: colors.mutedForeground }]}>Cerradas hoy</Text>
          </View>
          <View style={[styles.statCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Acciones recientes</Text>

        {recentActions.length === 0 ? (
          <View style={[styles.emptyCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Feather name="shield" size={28} color={colors.mutedForeground} />
            <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>
              Sin acciones aun. Inicia el monitoreo.
            </Text>
          </View>
        ) : (
            ...
            return (
              ...
              >
                ...
                <View style={styles.actionInfo}>
                  ...
                  <Text style={[styles.actionReason, { color: colors.mutedForeground }]} numberOfLines={1}>
                    {action.reason}
                  </Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 17, fontFamily: "Inter_700Bold", marginTop: 6 },
  emptyCard: {
    borderRadius: 14,
    padding: 28,
    alignItems: "center",
